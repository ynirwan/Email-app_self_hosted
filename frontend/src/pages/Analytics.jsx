  ...
  return (
    <div className="flex items-center gap-2 min-w-[90px]">
      <div className="flex-1 bg-gray-100 rounded-full h-1.5 overflow-hidden">
        <div className={`h-1.5 rounded-full ${colorClass}`} style={{ width: `${w}%` }} />
      </div>
      <span className="text-xs tabular-nums font-medium w-10 text-right">{pct(value)}</span>
    </div>
  );
}
...
function SortButton({ col, sortCol, sortDir, onSort, children }) {
  ...
  return (
    ...
    >
      {children}
      <span className={`text-gray-300 group-hover:text-gray-500 ${active ? 'text-gray-700' : ''}`}>
        {active ? (sortDir === 'asc' ? '↑' : '↓') : '↕'}
      </span>
    </button>
  );
...
export default function Analytics() {
  ...
  const excludedCount = allCampaigns.filter(c => c.status === 'draft' || c.status === 'failed').length;
  const baseCampaigns = allCampaigns.filter(c => c.status !== 'draft' && c.status !== 'failed');
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      let av, bv;
      switch (sortCol) {
        case 'sent':        av = a.analytics?.total_sent || 0;     bv = b.analytics?.total_sent || 0; break;
        case 'open_rate':   av = a.analytics?.open_rate || 0;      bv = b.analytics?.open_rate || 0; break;
        case 'click_rate':  av = a.analytics?.click_rate || 0;     bv = b.analytics?.click_rate || 0; break;
        case 'bounce_rate': av = a.analytics?.bounce_rate || 0;    bv = b.analytics?.bounce_rate || 0; break;
        case 'unsub_rate':  av = a.analytics?.unsubscribe_rate||0; bv = b.analytics?.unsubscribe_rate||0; break;
        default:            av = new Date(a.created_at||0);        bv = new Date(b.created_at||0);
      }
    ...
    <div className="space-y-7">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <p className="text-sm text-gray-500">
            ...
            {date_range && (
              <span> · {fmtD(date_range.start_date)} – {fmtD(date_range.end_date)}</span>
            )}
          </p>
          {excludedCount > 0 && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <SummaryCard
          ...
          icon="📬"
          valueColor={
            (summary?.average_open_rate || 0) >= 20 ? 'text-green-600' :
            (summary?.average_open_rate || 0) >= 10 ? 'text-yellow-600' : 'text-red-600'
          }
        />
        <SummaryCard
          ...
          icon="🖱️"
          valueColor={
            (summary?.average_click_rate || 0) >= 3 ? 'text-purple-600' :
            (summary?.average_click_rate || 0) >= 1 ? 'text-yellow-600' : 'text-gray-500'
          }
        />
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                <th className="px-5 py-3 text-left">
                  <SortButton col="created_at" sortCol={sortCol} sortDir={sortDir} onSort={handleSort}>
                    Campaign
                  </SortButton>
                </th>
                <th className="px-4 py-3 text-left w-24">Status</th>
                <th className="px-4 py-3 text-right">
                  <SortButton col="sent" sortCol={sortCol} sortDir={sortDir} onSort={handleSort}>
                    Sent
                  </SortButton>
                </th>
                <th className="px-4 py-3 text-left min-w-[130px]">
                  <SortButton col="open_rate" sortCol={sortCol} sortDir={sortDir} onSort={handleSort}>
                    Open Rate
                  </SortButton>
                </th>
                <th className="px-4 py-3 text-left min-w-[130px]">
                  <SortButton col="click_rate" sortCol={sortCol} sortDir={sortDir} onSort={handleSort}>
                    Click Rate
                  </SortButton>
                </th>
                {showAllMetrics && <>
                  <th className="px-4 py-3 text-left min-w-[130px]">
                    <SortButton col="bounce_rate" sortCol={sortCol} sortDir={sortDir} onSort={handleSort}>
                      Bounce
                    </SortButton>
                  </th>
                  <th className="px-4 py-3 text-left min-w-[130px]">
                    <SortButton col="unsub_rate" sortCol={sortCol} sortDir={sortDir} onSort={handleSort}>
                      Unsub
                    </SortButton>
                  </th>
                </>}
                <th className="px-4 py-3 text-right w-20 text-xs font-medium text-gray-500 uppercase tracking-wider">Date</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.length > 0 ? filtered.map((c) => {
                ...
                return (
                  <tr key={c._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_STYLE[c.status] || STATUS_STYLE.draft}`}>
                        {c.status || 'draft'}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <RateBar
                        ...
                        max={60}
                        colorClass={
                          (a.open_rate || 0) >= 20 ? 'bg-green-500' :
                          (a.open_rate || 0) >= 10 ? 'bg-yellow-400' : 'bg-gray-300'
                        }
                      />
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <RateBar
                        ...
                        max={20}
                        colorClass={
                          (a.click_rate || 0) >= 3 ? 'bg-purple-500' :
                          (a.click_rate || 0) >= 1 ? 'bg-blue-400' : 'bg-gray-300'
                        }
                      />
                    </td>
                    ...
                    {showAllMetrics && <>
                      <td className="px-4 py-3.5">
                        <RateBar
                          ...
                          max={10}
                          colorClass={(a.bounce_rate || 0) > 2 ? 'bg-red-500' : 'bg-gray-300'}
                        />
                      </td>
                      <td className="px-4 py-3.5">
                        <RateBar
                          ...
                          max={5}
                          colorClass={(a.unsubscribe_rate || 0) > 0.5 ? 'bg-orange-500' : 'bg-gray-300'}
                        />
                      </td>
                    </>}
                ...
                <tr>
                  <td colSpan={showAllMetrics ? 9 : 7} className="py-16 text-center">
                    {search ? (
                      <>
                        <p className="text-2xl mb-2">🔍</p>
                        <p className="text-sm font-medium text-gray-700">No campaigns match "{search}"</p>
                        <button onClick={() => setSearch('')} className="text-xs text-blue-600 mt-1 hover:underline">
                          Clear search
                        </button>
                      </>
                    ) : baseCampaigns.length === 0 && allCampaigns.length > 0 ? (
                      <>
                        <p className="text-2xl mb-2">📅</p>
                        <p className="text-sm font-medium text-gray-700">No campaigns in this date range</p>
                        <p className="text-xs text-gray-400 mt-1">Try extending the date range</p>
                      </>
                    ) : (
                      <>
                        <p className="text-3xl mb-2">📊</p>
                        <p className="text-sm font-medium text-gray-700">No campaigns yet</p>
                        <p className="text-xs text-gray-400 mt-1 mb-4">Create your first campaign to see analytics here</p>
                        <Link
                          to="/campaigns"
