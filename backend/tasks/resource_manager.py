...
from core.config import settings as app_settings, get_redis_key
from tasks.task_config import task_settings as settings
import redis
import json
...
class ResourceManager:
    ...
    def check_memory_usage(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            if not is_healthy:
                logger.warning(
                    f"High memory usage: {memory.percent}% (limit: {settings.MAX_MEMORY_USAGE_PERCENT}%)"
                )
    ...
    def can_process_batch(
        ...
        if settings.SKIP_HEALTH_CHECKS_FOR_TESTING:
            logger.info(
        ...
        if not settings.HEALTH_CHECK_STRICT_MODE:
    ...
    def cleanup_old_metrics(self):
        ...
        try:
            ...
            self._cleanup_keys_by_pattern(
                pattern, settings.METRICS_RETENTION_HOURS * 3600
            )
