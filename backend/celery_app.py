...
celery_app.conf.update(
    ...
    worker_prefetch_multiplier=max(
        1, task_settings.MAX_CONCURRENT_TASKS // 10
    ),  # Optimize prefetch
    ...
    task_queues=(
        Queue(
            "campaigns", Exchange("campaigns", type="direct"), routing_key="campaigns"
        ),
        Queue(
            "automation",
        ...
        Queue(
            "subscribers",
            Exchange("subscribers", type="direct"),
            routing_key="subscribers",
        ),
        Queue(
            "suppressions",
            Exchange("suppressions", type="direct"),
            routing_key="suppressions",
        ),
        ...
        Queue(
            "analytics", Exchange("analytics", type="direct"), routing_key="analytics"
        ),
        Queue(
            "templates", Exchange("templates", type="direct"), routing_key="templates"
        ),
    ...
    task_routes={
        ...
        "tasks.check_abandoned_cart_automations": {
            "queue": "automation",
            "priority": 6,
        },
        "tasks.check_inactive_subscriber_automations": {
            "queue": "automation",
            "priority": 5,
        },
    ...
    beat_schedule.update(
        {
            ...
            "collect-system-metrics": {
                ...
                "schedule": timedelta(
                    seconds=task_settings.METRICS_COLLECTION_INTERVAL_SECONDS
                ),
            ...
            "health-check": {
                ...
                "schedule": timedelta(
                    seconds=task_settings.HEALTH_CHECK_INTERVAL_SECONDS
                ),
            ...
            "provider-health-check": {
                ...
                "schedule": timedelta(
                    seconds=task_settings.PROVIDER_HEALTH_CHECK_INTERVAL_SECONDS
                ),
                "options": {"queue": "monitoring", "priority": 4},
            },
...
def task_postrun_handler(
    ...
    **extra,
    ...
    try:
        logger.debug(
            f"Task completed: {task.name if task else 'unknown'} [{task_id}] - State: {state}"
        )
    except Exception as e:
...
def worker_init_handler(sender=None, **kwargs):
    ...
    try:
        logger.info(
            f"🔧 Worker initializing: {sender.hostname if sender else 'unknown'}"
        )
    except Exception as e:
...
def verify_celery_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"   • Beat schedule: {config_info['beat_schedule_count']} periodic tasks"
        )
        logger.info(f"   • Included tasks: {config_info['included_tasks']} modules")
        logger.info(
            f"   • Production features: {'Enabled' if config_info['production_features'] else 'Basic'}"
        )
...
def get_celery_status() -> Dict[str, Any]:
    ...
    try:
        ...
        total_scheduled = sum(
            len(tasks) for tasks in status["scheduled_tasks"].values()
        )
