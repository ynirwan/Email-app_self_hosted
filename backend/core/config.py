...
class Settings:
    ...
    DB_CONNECTION_TIMEOUT_SECONDS: int = int(
        os.getenv("DB_CONNECTION_TIMEOUT_SECONDS", "10")
    )
    DB_SOCKET_TIMEOUT_SECONDS: int = int(os.getenv("DB_SOCKET_TIMEOUT_SECONDS", "30"))
    DB_SERVER_SELECTION_TIMEOUT_SECONDS: int = int(
        os.getenv("DB_SERVER_SELECTION_TIMEOUT_SECONDS", "10")
    )
    ...
    EMAIL_QUOTA_ENABLED: bool = (
        os.getenv("EMAIL_QUOTA_ENABLED", "false").lower() == "true"
    )
    ...
    STARTUP_RECOVERY_ENABLED: bool = (
        os.getenv("STARTUP_RECOVERY_ENABLED", "true").lower() == "true"
    )
    ...
    # ===== TASK / CAMPAIGN SETTINGS (referenced by tasks/campaign/*.py via `settings`) =====
    # Rate limiting
    RATE_LIMIT_WINDOW_SECONDS: int = int(os.getenv("RATE_LIMIT_WINDOW_SECONDS", "60"))
    RATE_LIMIT_SUCCESS_THRESHOLD: float = float(
        os.getenv("RATE_LIMIT_SUCCESS_THRESHOLD", "0.95")
    )
    RATE_LIMIT_FAILURE_THRESHOLD: float = float(
        os.getenv("RATE_LIMIT_FAILURE_THRESHOLD", "0.05")
    )
    ...
    SMTP_CIRCUIT_BREAKER_TIMEOUT_SECONDS: int = int(
        os.getenv("SMTP_CIRCUIT_BREAKER_TIMEOUT_SECONDS", "60")
    )
    ...
    METRICS_RETENTION_HOURS: int = int(os.getenv("METRICS_RETENTION_HOURS", "168"))

    # ===== DATABASE POOLING =====
    ENABLE_DATABASE_POOLING: bool = (
        os.getenv("ENABLE_DATABASE_POOLING", "true").lower() == "true"
    )
    ...
    def get_security_config(self) -> Dict[str, Any]:
        return {
            ...
            "jwt": {"algorithm": self.JWT_ALGORITHM, "expiry_seconds": self.JWT_EXP},
        }
