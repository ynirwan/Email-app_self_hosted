...
async def background_upload_enhanced(
    ...
    """
    Receives one chunk at a time. Does NOT start processing.
    Chunk 0: creates job, persists field_registry, writes chunk to disk.
    Chunk N: looks up existing job_id, writes chunk to disk.
    After all chunks are sent, frontend calls POST /background-upload/start/{job_id}.
    """
    total_records = len(payload.subscribers)
    if total_records == 0:
        raise HTTPException(status_code=400, detail="No subscribers provided")

    is_first_chunk = payload.job_id is None

    if is_first_chunk:
        # ── Concurrent upload guard ───────────────────────────────────────────
        existing = await get_jobs_collection().find_one(
            {
        ...
        if existing:
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": f"An upload is already active for '{payload.list_name}'",
                    "job_id": existing.get("job_id"),
                },
            )

        # ── Persist field registry ────────────────────────────────────────────
        ...
        # ── Create job ────────────────────────────────────────────────────────
        # total_records is a placeholder; real total is set in /start
        job_id = await job_manager.create_job(
            "background_upload", payload.list_name, total_records
        )
        logger.info(f"📦 Job {job_id} created for list '{payload.list_name}'")

    else:
        job_id = payload.job_id
        job = await get_jobs_collection().find_one({"_id": job_id})
        if not job:
            raise HTTPException(status_code=404, detail=f"Job {job_id} not found")
        if job.get("status") != "pending":
            raise HTTPException(
                status_code=409,
                detail=f"Job {job_id} is '{job.get('status')}' — cannot append chunks",
            )
    ...
    # Next chunk number = how many chunk files already exist
    existing_files = sorted(glob.glob(os.path.join(directory, "chunk_*.json")))
    chunk_n = len(existing_files)

    # Load registry dict — from this payload or from the persisted list doc
    registry_dict = (
    ...
    if registry_dict is None:
        try:
            ...
            lists_col = get_lists_collection()
            persisted = await lists_col.find_one(
                {"list_name": payload.list_name}, {"_id": 0}
            )
            registry_dict = persisted
        except Exception:
            pass
    ...
    chunks_on_disk = len(sorted(glob.glob(os.path.join(directory, "chunk_*.json"))))
    logger.info(
        f"📦 Job {job_id} | chunk {chunk_n} written "
        f"({total_records:,} records, {chunks_on_disk} total on disk)"
    )
    ...
    return {
        ...
        "chunk_number": chunk_n,
        "chunk_records": total_records,
        "total_chunks_received": chunks_on_disk,
...
@router.post(
    "/background-upload/start/{job_id}", dependencies=[Depends(rate_limit_check)]
)
async def start_upload_job(
    ...
    """
    Called by the frontend after all chunks have been POSTed.
    Counts records across chunk files, updates the job total, then
    kicks off save_upload_in_chunks → process_upload_chunks as a background task.
    """
    ...
    if job.get("status") != "pending":
        raise HTTPException(
            status_code=409,
            detail=f"Job already in status '{job.get('status')}'",
        )

    directory = os.path.join("upload_queue", "chunks", job_id)
    chunk_files = sorted(glob.glob(os.path.join(directory, "chunk_*.json")))

    if not chunk_files:
        await job_manager.mark_job_failed(job_id, "No chunk files found on disk")
        raise HTTPException(
            status_code=400, detail="No chunks found — upload chunks first"
        )

    # Count real total from files if frontend didn't pass it
    ...
    # process_upload_chunks is your existing function — it reads the chunk files
    # from disk and upserts into MongoDB.
    background_tasks.add_task(
        _process_chunks_background,
        job_id,
        job.get("list_name", ""),
        chunk_files,
        actual_total,
    ...
    logger.info(
        f"🚀 Job {job_id} started | {len(chunk_files)} chunks | {actual_total:,} records"
    )
...
async def _process_chunks_background(
    job_id: str, list_name: str, chunk_files: list, total_records: int
):
    """
    Thin wrapper that calls your existing process_upload_chunks and
    guarantees mark_job_failed is called on any unhandled exception.
    """
    try:
        await process_upload_chunks(job_id, list_name, chunk_files, total_records)
    except Exception as exc:
        logger.error(
            f"💥 _process_chunks_background unhandled: {exc}\n{traceback.format_exc()}"
        )
        await job_manager.mark_job_failed(job_id, f"Unexpected error: {exc}")
    finally:
        ...
        try:
            ...
            shutil.rmtree(
                os.path.join("upload_queue", "chunks", job_id), ignore_errors=True
            )
        except Exception:
            pass
...
async def delete_list_enhanced(
    ...
    try:
        ...
        logger.error(f"âŒ Enhanced list deletion failed for '{list_name}': {e}")
...
async def delete_single_job(job_id: str):
    """
    Delete a single job document by ID.
    Called by the frontend Clear button on failed jobs in the banner.
    """
    try:
        jobs_collection = get_jobs_collection()
