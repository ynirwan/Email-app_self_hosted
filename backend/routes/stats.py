...
def _serialize(obj):
    """Recursively convert ObjectId → str so FastAPI can JSON-encode the response."""
...
async def fetch_quick_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"total_subscribers": 0, "total_campaigns": 0, "error": str(e)}
...
async def fetch_dashboard_summary() -> Dict[str, Any]:
    ...
    try:
            ...
            campaigns_collection.aggregate(
                [
                    ...
                    {"$limit": 10},
                    {
                        "$project": {
                            "title": 1,
                            # FIX: expose both counters so frontend can add them; avoid ObjectId in _id
                            "_id": {"$toString": "$_id"},
                            "sent_count": 1,
                            "delivered_count": 1,
                            # total_sent = sent + delivered (webhooks promote "sent" → "delivered")
                            "total_sent": {
                                "$add": [
                                    {"$ifNull": ["$sent_count", 0]},
                                    {"$ifNull": ["$delivered_count", 0]},
                                ]
                            },
                            "open_rate": 1,
                            "click_rate": 1,
        ...
        for status in campaign_statuses:
            status_name = (
                status.get("_id", "").lower() if status.get("_id") else "unknown"
            )
        ...
        if recent_campaigns:
            open_rates = [
                c.get("open_rate", 0) for c in recent_campaigns if c.get("open_rate")
            ]
            click_rates = [
                c.get("click_rate", 0) for c in recent_campaigns if c.get("click_rate")
            ]

            avg_open_rate = (
                round(sum(open_rates) / len(open_rates), 2) if open_rates else 0
            )
            avg_click_rate = (
                round(sum(click_rates) / len(click_rates), 2) if click_rates else 0
            )

        # Return format matching Dashboard.jsx expectations
        # _serialize() converts any ObjectId in top_lists/_id or recent_campaigns/_id → str
        return {
            ...
            "lists": _serialize(top_lists),
            "recent_performance": _serialize(recent_campaigns[:5]),
            "timestamp": datetime.utcnow().isoformat(),
        }
        ...
        return {
            ...
            "failed_campaigns": 0,
            "summary": {"active_rate": 0, "avg_open_rate": 0, "avg_click_rate": 0},
            "lists": [],
            "recent_performance": [],
            "error": str(e),
        }
        ...
        if result:
            ...
            return {
                ...
                "engagement_rate": round(
                    (status_counts["active"] / total * 100) if total > 0 else 0, 2
                ),
                "churn_rate": round(
                    (status_counts["unsubscribed"] / total * 100) if total > 0 else 0, 2
                ),
                "timestamp": datetime.utcnow().isoformat(),
            }
    ...
    try:
                ...
                "$facet": {
                    ...
                    "performance": [
                        {"$match": {"status": "completed"}},
                        {
                            "$group": {
                                "_id": None,
                                # FIX: sum sent_count + delivered_count to avoid under-counting
                                # finalize_campaign sets sent_count from logs where latest_status == "sent"
                                # but webhooks may have promoted some to "delivered" — count both
                                "total_sent": {
                                    "$sum": {
                                        "$add": [
                                            {"$ifNull": ["$sent_count", 0]},
        ...
        if result:
            ...
            return {
                ...
                "recent_campaigns": _serialize(data["recent"]),
                "timestamp": datetime.utcnow().isoformat(),
            }
...
async def fetch_engagement_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        pipeline = [
            {"$match": {"status": "completed", "sent_at": {"$gte": thirty_days_ago}}},
            {
                "$group": {
                    "_id": None,
...
async def get_dashboard_summary(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        background_tasks.add_task(
            StatsCache.set, CacheKeys.SUMMARY, data, CacheConfig.SUMMARY
        )
...
async def get_subscriber_stats(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Cache in background
        background_tasks.add_task(
            StatsCache.set, CacheKeys.SUBSCRIBERS_DETAIL, data, CacheConfig.SUBSCRIBERS
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to load subscriber statistics"
        )
...
async def get_campaign_stats(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Cache in background
        background_tasks.add_task(
            StatsCache.set, CacheKeys.CAMPAIGNS_DETAIL, data, CacheConfig.CAMPAIGNS
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to load campaign statistics"
        )
...
async def get_engagement_stats(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Cache in background
        background_tasks.add_task(
            StatsCache.set, CacheKeys.ENGAGEMENT, data, CacheConfig.ANALYTICS
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to load engagement statistics"
        )
...
async def get_cache_status():
    ...
    try:
        redis_client = get_redis()
        if not redis_client:
            return {"cache_enabled": False, "message": "Redis not configured"}
...
async def invalidate_stats_on_change(change_type: str = "all"):
    ...
    if change_type == "subscriber":
        await StatsCache.delete(
            CacheKeys.QUICK, CacheKeys.SUMMARY, CacheKeys.SUBSCRIBERS_DETAIL
        )
