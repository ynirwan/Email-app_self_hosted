...
async def get_campaign_analytics(campaign_id: str):
    ...
    try:
        ...
        # Calculate rates.
        # total_sent = campaign.sent_count (authoritative counter updated by Celery task)
        total_sent = campaign.get("sent_count", 0) or 0

        # Fallback: if analytics counters are 0, count directly from email_events.
        # This handles the window between pixel hits and analytics writes, or
        # partial failures where _increment_analytics didn't fire.
        ...
        if total_sent > 0:
            ...
            analytics["open_rate"]        = round(total_opened  / total_sent * 100, 1)
            analytics["click_rate"]       = round(total_clicked / total_sent * 100, 1)
        ...
        async for event in events_collection.find(
            {
                "campaign_id": ObjectId(campaign_id),
                "event_type": {"$in": ["opened", "clicked", "unsubscribed", "bounced", "delivered", "spam_report"]},
            }
...
async def get_analytics_dashboard(days: int = Query(default=30, ge=1, le=365)):
    ...
    try:
        ...
        async for campaign in campaigns_collection.find(
            ...
            # total_sent = sent_count from the campaign doc (always accurate)
            ...
            if analytics:
                ...
                # Always use campaign.sent_count as the authoritative total_sent
                analytics["total_sent"] = camp_sent
                # Recompute rates with correct denominator
                if camp_sent > 0:
                    analytics["open_rate"]  = round(analytics.get("total_opened",  0) / camp_sent * 100, 2)
                    analytics["click_rate"] = round(analytics.get("total_clicked", 0) / camp_sent * 100, 2)
        ...
        # Summary metrics — use campaign.sent_count as authoritative
...
async def get_campaign_openers(
    ...
    Each row includes email, IP, user-agent, device guess, and timestamp.
    """
    try:
...
async def get_campaign_clickers(
    ...
    Each row includes email, URL clicked, IP, device, and timestamp.
    """
    try:
        col = get_email_events_collection()
        pipeline = [
...
def _parse_device(ua: str) -> str:
    ...
    if any(x in ua_lower for x in ("ipad", "tablet")):
        return "Tablet"
    if any(x in ua_lower for x in ("mozilla", "chrome", "safari", "firefox", "edge", "opera", "msie")):
        return "Desktop"
    return "Unknown"
