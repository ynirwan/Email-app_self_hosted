...
from routes import (
    ...
    ab_testing, automation, events, automation_analytics, audit
)
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"🎯 Production features enabled: {len(enabled_features)}/{len(PRODUCTION_FEATURES)}")
    ...
    if PRODUCTION_FEATURES.get('config'):
        try:
            ...
            logger.info(f"Production readiness: {'✅ READY' if ready else '⚠️  NOT READY'}")
            for check in checks:
                logger.info(f"   {check}")
        except Exception as e:
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                ...
                "message": str(e) if settings.DEBUG_MODE else "An unexpected error occurred",
...
async def worker_status():
    ...
    try:
        ...
        return {
            "error": str(e),
            "message": "Worker status not available"
        }
...
async def root():
    ...
    return {
        ...
        "environment": settings.ENVIRONMENT,
        "status": "operational",
        "documentation": "/docs" if settings.DEBUG_MODE else "Contact support for API documentation",
        "health_check": "/health",
        "endpoints": {
...
# Email Settings
app.include_router(email_settings.router, prefix="/api/email", tags=["Email Settings"])
